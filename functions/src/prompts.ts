...
export function _storyFormatBlock(featureId: string): string {
...
export function getUserStoryProposalPrompt(summaryContent: string, userStoryContent: string, featureId: string): string {
...
export function getTaskProposalPrompt(userStoryContent: string, currentTasks: Array<Record<string, unknown>>, featureId: string, technicalNotes: string): string {
...
export function getFeatureIdentificationPrompt(summaryContent: string, knownFeatures: Array<{id: string; summary: string}>): string {
...
export function getFeatureDocEditsPrompt(currentContent: string, changeSummary: string[], featureId: string): string {
  var changesText = changeSummary.map(function(c: string, i: number) { return (i + 1) + '. ' + c; }).join('\n');
...
export function getNormalizationPrompt(structuredDoc: string, featureId: string, comments: Array<{quotedText?: string; content: string}>): string {
  var commentsSection = '';
...
export function getPatchPlanPrompt(normalizedDoc: Record<string, unknown>, meetingSummary: string, featureId: string, comments: Array<{quotedText?: string; content: string}>): string {
  var commentsSection = '';
  if (comments && comments.length) {
 ...
 */
export function formatTasksSummary(tasks: Array<Record<string, unknown>>): string {
