---
name: run-task
description: Pick up the next Ready task from the Google Sheet and run it through the full agent workflow (investigate → implement → review → PR).
user_invocable: true
---
...
2. **Invoke the Orchestrator Agent**
   ...
   - Run investigator → implementer → reviewer cycle
   - Create PR
